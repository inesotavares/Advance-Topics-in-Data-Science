...
# Handeling the inf value
features$weekday_vs_weekend_ratio[3]<-1e+4
...
features_scaled<-as.data.frame(features_scaled[,-1])
row.names(features_scaled)<-gsub(".csv", "", features$id)
set.seed(25)
...
dist <- daisy(features_scaled, metric = "gower")
dim(dist)
hc <- hclust(dist) # Hierarchical clustering
plot(hc)
cutree_clusters <- cutree(hc, k = k) # Cut the tree to get k clusters
plot(hc)
rect.hclust(hc,k)
...
fviz_dend(hc, k = k, cex = 0.6, main = "Dendrogram for Hierarchical Clustering")
